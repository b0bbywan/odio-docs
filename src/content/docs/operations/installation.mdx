---
title: How to install odio on a Raspberry Pi or Debian
description: Flash an odio image with Raspberry Pi Imager or run the curl installer on Debian or Ubuntu. Pick a target user and optional sources like AirPlay and Spotify.
howto: true
---
...
- **Flash an image** — use [Raspberry Pi Imager](https://www.raspberrypi.com/software/):
  ...
  The odio stack always runs under the `odio` user. In Imager settings, **pick `odio` as the user** so the upcoming UI-driven upgrade is wired in by default. A different user is fine for SSH, and the CLI [upgrade flow](/operations/upgrade/) is independent of the choice.
...
The installer is idempotent, re-run it to upgrade an existing installation. Since [2026.4.1rc1](https://github.com/b0bbywan/odios/releases/tag/2026.4.1rc1), each node also shows available updates in its SSH MOTD. See [Upgrade](/operations/upgrade/) for details and the upcoming targeted-upgrade flow.
