...
| Raspberry Pi 2 | armv7 | Flash, curl | Raspberry Pi OS Lite (Trixie) |
...
- **Flash an image** — use [Raspberry Pi Imager](https://www.raspberrypi.com/software/):
  ...
  The odio stack always runs under the `odio` user. In Imager settings, **pick `odio` as the user** so the UI-driven upgrade is wired in by default. A different user is fine for SSH, and the CLI [upgrade flow](/operations/upgrade/) is independent of the choice.
