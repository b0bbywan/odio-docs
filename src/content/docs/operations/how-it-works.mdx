---
title: The architecture and design behind odio
description: 'How odio works: a systemd user session, PulseAudio mixing instead of ALSA, and the go-odio-api REST API that controls every source over MPRIS on D-Bus.'
---

import EcosystemTable from '../../../components/EcosystemTable.astro';
...
Every audio service (MPD, shairport-sync, spotifyd, upmpdcli, Snapclient) runs as a systemd user unit. They can be started, stopped, and monitored through the API: no SSH or reboot needed.
...
odio ships without equalizer or volume normalization, I don't use them and prefer a clean, unprocessed signal. If there is demand and concrete proposals, this could be considered ([open an issue](https://github.com/b0bbywan/odios/issues)). You can also set up your own as an [extension](/operations/extensions)

### Mono output

odio outputs stereo. For a single speaker, PulseAudio's `module-remap-sink` mixes both channels down into a mono sink placed on top of your DAC. To try it, from an SSH session as the odio user:
...
The **Mono** sink also shows up under **Audio Server → Default Sink** in the [embedded web UI](/control/embedded-ui/), where you can switch between it and the stereo output like any other sink. The module does not survive a reboot. A small script and a user unit that load it at boot, and restore stereo when stopped, are on [odios#86](https://github.com/b0bbywan/odios/discussions/86), opened by [@langmario](https://github.com/langmario) for a DigiAMP+ driving one speaker, which also tracks a mono/stereo toggle in odio-api.
...
odio picks a different middle: a minimal base, tuned to run well on a Pi B+, with software and defaults chosen to cover the common cases with a deliberately small [settings page](/operations/settings/). It holds what genuinely needs tweaking on a node, upgrades, which components run, and the DAC overlay, and is meant to grow from there. Everything else, overclocking included, is left to you over SSH, where the standard Linux tools already work the way you'd expect.
