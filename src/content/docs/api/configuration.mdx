---
title: Configure the odio API with config.yaml drop-ins
description: Configure go-odio-api through config.yaml and conf.d drop-ins, covering bind, port, CORS, and each backend, plus Bluetooth, Power, and Zeroconf setup.
howto: true
---
...
<Aside type="caution">On installer-managed systems, changes to `config.yaml` may be overwritten on update. For persistent customization, add a `*.yml` drop-in in `~/.config/odio-api/conf.d/` instead: it is merged on top of `config.yaml` at startup and left untouched by the installer. See [Drop-in overrides](#drop-in-overrides).</Aside>
...
| `ui.enabled` | `true` | Serves the [embedded web UI](/control/embedded-ui/) at `/ui`. Auto-disabled at startup if `bind` does not include `lo` (the UI is loopback-only by design). |
| `ui.admin` | unset | Optional link to an admin web app, shown as a gear icon in the UI header. Same forms as a service `url` in the [systemd whitelist](#systemd-opt-in-whitelist-required): `":8021"` resolves against the node's own address. odios points it at the [settings page](/operations/settings/). Since [v0.17.0](https://github.com/b0bbywan/go-odio-api/releases/tag/v0.17.0). |
| `sse.enabled` | `true` | Serves the [SSE event stream](/api/events/) at `/events`. The embedded UI and the [PWA](/control/pwa/) both rely on it for real-time updates. |
| `cors.origins` | PWA defaults | Browser origins allowed to call the API, see below. |

### CORS

The default `cors.origins` covers the two hosted [PWA](/control/pwa/) addresses:

- `https://pwa.odio.love`
- `https://odio-pwa.vercel.app`

<Aside type="caution">
Setting `cors.origins` **replaces** the defaults, it is not merged. To keep the hosted PWA reachable while adding your own origin (for example when [self-hosting the PWA](/control/pwa/#self-hosting)), list everything explicitly:
...
On odio streamer installs, the [installer](https://github.com/b0bbywan/odios) creates this directory and drops a `10-systemd.yml.default` reference matching the host's installed services, regenerated on every upgrade run. The `.default` extension keeps it ignored at runtime. To use it as a starting point, copy it under a `.yml` name and edit:
...
Since odio-api v0.12.0, each whitelist entry is a map with a `name` and an optional `url`. The URL is exposed on the [/services](/api/systemd/) response so clients (the [embedded UI](/control/embedded-ui/), [PWA](/control/pwa/), Home Assistant) can render a direct link, which is how myMPD shows up alongside `mpd.service` on installs that ship it.
...
`AutoEnable=false` keeps the adapter off at boot — power is managed by the API (or Home Assistant). To have the API power it on when the service starts, set `bluetooth.powerOnStart: true` (since v0.13.1).
