...
- **[mpDris2](https://github.com/b0bbywan/mpDris2/blob/master/.github/workflows/build.yml)** and **[snapclientmpris](https://github.com/b0bbywan/snapclientmpris/blob/master/.github/workflows/build.yml)** share an identical Python pipeline: `ruff`, `mypy`, and `pytest` on Python 3.11, driven through `Makefile` targets (`make lint-ruff`, `make lint-mypy`, `make test`) so local dev and CI invoke the same commands. On `v*` tags both run `make check-tag` to fail fast when the git tag and the package's `__init__.py` disagree, before the `.deb` job spins up.
...
Six upstream repos feed `.deb` into [apt.odio.love](https://apt.odio.love): the two Go components built in-house ([go-odio-api](https://github.com/b0bbywan/go-odio-api), [go-mpd-discplayer](https://github.com/b0bbywan/go-mpd-discplayer)), the [spotifyd](https://github.com/b0bbywan/spotifyd) fork, [odio-mympd](https://github.com/b0bbywan/odio-mympd), and the two pure-Python rewrites [mpDris2](https://github.com/b0bbywan/mpDris2) and [snapclientmpris](https://github.com/b0bbywan/snapclientmpris). The flow is:

1. A `v*` tag gets pushed.
2. The repo's `build.yml` produces the `.deb`. Most components cross-compile for amd64, arm64, armv6, and armv7, packed via [nfpm](https://nfpm.goreleaser.com/) (Go components) or `dpkg-buildpackage` inside per-arch builder images (odio-mympd); mpDris2 and snapclientmpris are pure Python, so a single `arch:all` `.deb` is built once inside a `debian:trixie` container (mpDris2 also publishes an sdist tarball alongside). go-odio-api also packs `.rpm`.
