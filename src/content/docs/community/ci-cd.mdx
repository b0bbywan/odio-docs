...
- **[snapclientmpris](https://github.com/b0bbywan/snapclientmpris/blob/master/.github/workflows/build.yml)** runs `ruff`, `mypy`, and `pytest` on Python 3.11 via the repo's [`Makefile`](https://github.com/b0bbywan/snapclientmpris/blob/master/Makefile) targets (`make lint-ruff`, `make lint-mypy`, `make test`), so local dev and CI invoke the same commands. On `v*` tags it also runs `make check-tag` to fail fast when the git tag and `snapclientmpris/__init__.py` disagree, before the `.deb` job spins up.
...
Six upstream repos feed `.deb` into [apt.odio.love](https://apt.odio.love): the two Go components built in-house ([go-odio-api](https://github.com/b0bbywan/go-odio-api), [go-mpd-discplayer](https://github.com/b0bbywan/go-mpd-discplayer)), the [spotifyd](https://github.com/b0bbywan/spotifyd) and [mpDris2](https://github.com/b0bbywan/mpDris2) forks, [odio-mympd](https://github.com/b0bbywan/odio-mympd), and the pure-Python [snapclientmpris](https://github.com/b0bbywan/snapclientmpris). The flow is:

1. A `v*` tag gets pushed.
2. The repo's `build.yml` produces the `.deb`. Most components cross-compile for amd64, arm64, armv6, and armv7, packed via [nfpm](https://nfpm.goreleaser.com/) (Go components) or `dpkg-buildpackage` inside per-arch builder images (mpDris2, odio-mympd); snapclientmpris is pure Python, so a single `arch:all` `.deb` is built once inside a `debian:trixie` container. go-odio-api also packs `.rpm`.
...
| [odios](https://github.com/b0bbywan/odios) | ansible-lint, shellcheck, playbook + install + upgrade tests | Installer tarball, Pi images, Imager manifest | Orchestrator. Adds `apt.odio.love` as a source, installs `odio-api`, `mpd-discplayer`, `spotifyd`, `mpdris2`, `mympd`, `snapclientmpris` |
...
| [odio-apt-repo](https://github.com/b0bbywan/odio-apt-repo) | receives dispatches, weekly cron | `apt.odio.love` via GitHub Pages | Ingests `.deb` from `go-odio-api`, `go-mpd-discplayer`, [spotifyd](https://github.com/b0bbywan/spotifyd), [mpDris2](https://github.com/b0bbywan/mpDris2), [snapclientmpris](https://github.com/b0bbywan/snapclientmpris), and [odio-mympd](https://github.com/b0bbywan/odio-mympd) |
