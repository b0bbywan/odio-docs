...
- Runs an upgrade matrix on `arm64` against four pre-provisioned baselines (`2026.4.0rc3`, `2026.4.1rc1`, `2026.4.1rc2`, `2026.4.2b1`), one per stage of the upgrade-mechanism rollout, so every supported upgrade path is exercised before publication. Baselines live as Docker images at `ghcr.io/b0bbywan/odios/test-baseline:<tag>-<arch>`, rebuilt once on demand via [`test-baseline-image.yml`](https://github.com/b0bbywan/odios/blob/main/.github/workflows/test-baseline-image.yml).
