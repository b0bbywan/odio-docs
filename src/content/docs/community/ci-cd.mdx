...
- **[odios](https://github.com/b0bbywan/odios/tree/main/.github/workflows)** runs a unified [`checks.yml`](https://github.com/b0bbywan/odios/blob/main/.github/workflows/checks.yml) workflow combining `ansible-lint`, `shellcheck` (installer, test, image-builder, and `scripts/`), Python `unittest` against `tests/`, and Python lint with [ruff](https://docs.astral.sh/ruff/) + [mypy](https://mypy.readthedocs.io/) on the progress callback plugin and the manifest scripts. It exposes a `workflow_call` trigger so the release pipeline gates publishing on the very same checks.
...
Seven upstream repos feed `.deb` into [apt.odio.love](https://apt.odio.love): the three Go components built in-house ([go-odio-api](https://github.com/b0bbywan/go-odio-api), [odioctl](https://github.com/b0bbywan/odioctl), [go-mpd-discplayer](https://github.com/b0bbywan/go-mpd-discplayer)), the [spotifyd](https://github.com/b0bbywan/spotifyd) fork, [odio-mympd](https://github.com/b0bbywan/odio-mympd), and the two pure-Python rewrites [mpd2mpris](https://github.com/b0bbywan/mpd2mpris) and [snapclientmpris](https://github.com/b0bbywan/snapclientmpris). The flow is:

1. A `v*` tag gets pushed.
2. The repo's `build.yml` produces the `.deb`. Most components cross-compile for amd64, arm64, armv6, and armv7, packed via [nfpm](https://nfpm.goreleaser.com/) (Go components) or `dpkg-buildpackage` inside per-arch builder images (odio-mympd); mpd2mpris and snapclientmpris are pure Python, so a single `arch:all` `.deb` is built once inside a `debian:trixie` container (mpd2mpris also publishes an sdist tarball alongside). go-odio-api also packs `.rpm`.
...
- `gh release list` resolves the latest stable and latest prerelease for the tracked repos: `go-odio-api`, `odioctl`, `go-mpd-discplayer`, `spotifyd`, `mpd2mpris`, `odio-mympd`, `snapclientmpris`. Manual `workflow_dispatch` accepts an explicit version override per repo.
...
| [odios](https://github.com/b0bbywan/odios) | ansible-lint, shellcheck, playbook + install + upgrade tests | Installer tarball, Pi images, Imager manifest | Orchestrator. Adds `apt.odio.love` as a source, installs `odio-api`, `mpd-discplayer`, `spotifyd`, `mpd2mpris`, `mympd`, `snapclientmpris` |
...
| [mpd2mpris](https://github.com/b0bbywan/mpd2mpris) | ruff, mypy, pytest, `make check-tag` on `v*` | `.deb` (`arch:all`), sdist tarball | MPRIS2 D-Bus bridge for MPD. Complete rewrite around `python-mpd2` and `dbus-fast`, with [`as_directory`-aware](/disc-player/configuration/#cue-sheet-support) cover-art lookup for virtual CUE folders. Ingested into `apt.odio.love` |
| [snapclientmpris](https://github.com/b0bbywan/snapclientmpris) | ruff, mypy, pytest, `make check-tag` on `v*` | `.deb` (`arch:all`) | MPRIS2 D-Bus bridge for the local Snapcast client. Forwards Play/Pause/Next/Previous to the snapserver source so multi-room pauses pause every listener. Ingested into `apt.odio.love` |
...
| [odio-apt-repo](https://github.com/b0bbywan/odio-apt-repo) | receives dispatches, weekly cron | `apt.odio.love` via GitHub Pages | Ingests `.deb` from `go-odio-api`, `go-mpd-discplayer`, [spotifyd](https://github.com/b0bbywan/spotifyd), [mpd2mpris](https://github.com/b0bbywan/mpd2mpris), [snapclientmpris](https://github.com/b0bbywan/snapclientmpris), and [odio-mympd](https://github.com/b0bbywan/odio-mympd) |
