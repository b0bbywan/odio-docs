...
The odio user's home directory is tiny, a handful of kilobytes. Two folders hold your customizations, `~/.config` and `~/.cache`, plus `/boot/firmware/config.txt` for the DAC overlay (managed from the [settings page](/operations/settings/#dac)) and overclocking. Back those up, and you can wipe the whole system, reflash, and be back where you were in minutes.

Which means you can actually try things. Install an [extension](/operations/extensions/), listed or not. Poke at a systemd unit. Tweak a software config file. Most of them should hold basic documentation and unused config options to explore. If you break it and can't fix it, you reflash and move on, a little sharper than before.

## A platform, not a product

odio isn't one monolithic app, it's independent components talking standard protocols: MPD, DLNA, MPRIS, D-Bus, [SSE events](/api/events/). You're not configuring a product, you're composing a system. The [Home Assistant](/control/home-assistant/), [Music Assistant](/guides/music-assistant/), and [HTPC](/guides/use-case-htpc/) setups are all just other pieces plugged onto the same surface.

The same doors are open to you. Write a script against the [API](/api/overview/). Build an [extension](/operations/extensions/). Pipe another source into the [TCP sink](/guides/network-audio/). Create something that didn't exist before, because nothing in odio was designed to stop you. Quite the opposite, really.
...
Every feature that came from someone other than the maintainer keeps their name on it. The [webradios integration](/guides/webradios/) wouldn't look the way it does without [@pbattino](https://github.com/pbattino). PRs keep their author, discussions credit whoever shaped the direction, [extensions](/operations/extensions/) link back to the people running them. The full roll lives in the [contributors list](https://github.com/b0bbywan/odios#contributors-) on the odios README, built with the [all-contributors](https://allcontributors.org/) spec so that any kind of help, code or not, gets a spot.
...
- Start with [installation](/operations/installation/) and [how it works](/operations/how-it-works/) to get the lay of the land.
- Browse [extensions](/operations/extensions/) to see what plugs in where.
