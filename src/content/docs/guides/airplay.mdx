...
import airplayUi from '../../../assets/airplay-embedded-ui.png';
...
<Image src={airplayUi} alt="odio embedded UI with Shairport Sync as the active media player, showing cover art, track metadata and playback controls" />

Before [2026.9.0b3](https://github.com/b0bbywan/odios/releases/tag/2026.9.0b3), shairport-sync published its MPRIS interface on the system D-Bus, where the odio API does not look, so AirPlay sessions never showed up. [Mykola (@VeeM)](https://github.com/VeeM) tracked it down and moved it to the session bus in [odios#91](https://github.com/b0bbywan/odios/pull/91). Upgrade an existing node to pick up the fix, see [Upgrade](/operations/upgrade/).
