...
![The settings page with the Log in to Tidal dialog open over the components list: the helper's output announces where the credentials will be written and that the code expires in 300 seconds, above a Tidal sign-in page button, and the Tidal row behind it carries the same link marked valid 5 minutes](../../../assets/settings-tidal-login.webp)
...
Qobuz plays two ways. [Qobuz Connect](/guides/qobuz-connect/) makes the node a device in the official Qobuz app, added in [2026.9.0b1](https://github.com/b0bbywan/odios/releases/tag/2026.9.0b1), enabled and signed in from the [settings page](/operations/settings/#components). The upmpdcli plugin adds Qobuz to the node's UPnP library instead; it is configured in the same file as Tidal, through its [own set of parameters](https://www.lesbonscomptes.com/upmpdcli/pages/upmpdcli-manual.html#_qobuz_streaming_service_parameters). We don't have a tested walkthrough yet; if you have set up Qobuz with odio, help us improve this page — [open an issue](https://github.com/b0bbywan/odios/issues).
