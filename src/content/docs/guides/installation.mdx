...
- **Flash an image** — use [Raspberry Pi Imager](https://www.raspberrypi.com/software/):
  ...
  The odio stack always runs under the `odio` user. In Imager settings, you can configure any user you want for SSH access, or simply use the `odio` user for both.
...
Upgrading an existing odio installation means re-running the installer. It is fully idempotent, it can be run on a fresh system or an existing one, and re-run safely at any time to update or repair the installation.

```bash
curl -fsSL https://beta.odio.love/install | bash
```

When a configuration file has been modified, the installer automatically creates a `<config>.bak` backup before applying changes. If the file ends up identical, no backup is kept. This applies to `/etc/bluetooth/main.conf`, `~/.config/mpd/mpd.conf`, `~/.config/pipewire/pipewire-pulse.conf`, `~/.config/shairport-sync/shairport-sync.conf`, and `~/.config/upmpdcli/upmpdcli.conf`.

To install a specific version or pre-release, set the `ODIOS_VERSION` variable:

```bash
# Stable version
curl -fsSL https://github.com/b0bbywan/odios/releases/download/2026.3.0/install.sh | ODIOS_VERSION=2026.3.0 bash

# Release candidate
curl -fsSL https://github.com/b0bbywan/odios/releases/download/2026.3.0rc1/install.sh | ODIOS_VERSION=2026.3.0rc1 bash
```

<Aside type="note">A more user-friendly upgrade system with automatic detection and targeted role upgrades, driven from the [odio App](/guides/pwa/), is being designed. See [RFC: Targeted upgrade system for odio](https://github.com/b0bbywan/odios/discussions/34) and join the discussion.</Aside>
