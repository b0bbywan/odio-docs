...
<Aside type="note">Installation as a standalone app requires a browser that supports PWA (Chrome, Edge, Safari, Firefox mobile). On Firefox desktop, the app works in the browser but cannot be added to the Start menu (Windows)/to the Dock (Mac). On some other mobile browsers, like DuckDuckGo, the option is available from the three-dot menu.</Aside>
