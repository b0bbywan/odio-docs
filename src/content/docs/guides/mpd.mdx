...
import tracklistView from '../../../assets/embedded-ui-tracklist.webp';
...
Since [mpd2mpris v0.13.0](https://github.com/b0bbywan/mpd2mpris/releases/tag/v0.13.0), the MPD queue is also exposed as an MPRIS [TrackList](/api/mpris/#tracklist) and stored playlists through the `Playlists` interface, so a client can browse the queue, jump to a track, add or remove entries, and switch playlists without speaking the MPD protocol. `AddTrack`, `RemoveTrack` and `GoTo` map to MPD's `addid`, `deleteid` and `playid`, and the URI schemes and MIME types advertised over MPRIS come from MPD's own `urlhandlers` and `decoders`. `ActivePlaylist` follows MPD's `lastloadedplaylist`, which needs MPD 0.24 or newer, everything else works on any supported MPD.
...
<div class="screenshot-carousel wide">
	...
	<figure>
		<Image src={tracklistView} alt="odio embedded UI with the media player card showing the MPD queue instead of the cover: four rows with a remove cross each, the playing one highlighted in green, transport controls below" />
		<figcaption>The MPD queue exposed as an MPRIS tracklist, browsable and editable from the embedded UI</figcaption>
	</figure>
</div>
...
It covers the MPD-side use cases the embedded UI doesn't: library browsing, tag-aware search, building a queue from scratch.
