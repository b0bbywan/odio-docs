...
- The third `bind_to_address` is a user socket; mpd2mpris connects to it (see below).
...
For the NAS MPD to appear as an MPRIS player (and for cover art to resolve), run [mpd2mpris](https://github.com/b0bbywan/mpd2mpris) on the NAS. It ships from [apt.odio.love](/operations/apt-repository/):

```bash
sudo apt install mpd2mpris
```

Configure `~/.config/mpd2mpris/mpd2mpris.conf`:
...
Connecting via the **unix socket** gives mpd2mpris local filesystem access, so cover-art lookups read `cover.*` files straight off the disk, including non-standard names like `folder.jpg`. Connecting over TCP (the LAN address) works too, but then mpd2mpris falls back to MPD's `albumart` command, which resolves only the standard `cover.{png,jpg,jxl,webp}` names and ships the bytes through MPD.
