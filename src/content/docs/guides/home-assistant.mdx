...
A node can be a full [odio streamer](/guides/installation/) on a Raspberry Pi, or just [go-odio-api](/api/installation/) running on any Linux machine with a D-Bus user session (desktop, NAS, laptop). Same integration either way, HA exposes whichever capabilities the node actually has.
